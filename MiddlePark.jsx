/* Scene — Middle Park page. */
const { useState: useStateMP, useEffect: useEffectMP } = React;
const MIDDLEPARK_VIDEO = "https://d8j0ntlcm91z4.cloudfront.net/user_38xzZboKViGWJOttwIXH07lWA1P/hf_20260405_170732_8a9ccda6-5cff-4628-b164-059c500a2b41.mp4";

function MiddleParkScene({ building, onBack }) {
  const [mounted, setMounted] = useStateMP(false);
  ...
  return (
    <div className="fixed inset-0 overflow-hidden no-tap-highlight" style={{ background: '#0a0b0e' }}>
      <video className="absolute inset-0 h-full w-full object-cover" src={MIDDLEPARK_VIDEO} autoPlay loop muted playsInline />
      ...
      <div
        ...
        <div className="mt-1 text-3xl font-semibold tracking-tight text-white md:text-4xl" style={{ textShadow: '0 2px 20px rgba(0,0,0,.6)' }}>
          {building ? building.name : 'Middle Park'}
          <div><span style={{ color: '#a8f8ac' }}>Revision Tool</span></div>
        </div>
        <div className="mt-1 text-sm text-white/60">{building ? building.sub : ''}</div>
      </div>
    </div>
  );
}

window.MiddleParkScene = MiddleParkScene;
