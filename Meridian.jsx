/* Scene — Meridian page. */
const { useState: useStateMeridian, useEffect: useEffectMeridian } = React;
const MERIDIAN_VIDEO = "https://res.cloudinary.com/dcytepiva/video/upload/v1780992288/Guy_typing_on_computer_202606082141_yszxca.mp4?_s=public-apps";

function MeridianScene({ building, onBack }) {
  const [mounted, setMounted] = useStateMeridian(false);
  ...
  return (
    <div className="fixed inset-0 overflow-hidden no-tap-highlight" style={{ background: '#0a0b0e' }}>
      <video className="absolute inset-0 h-full w-full object-cover" src={MERIDIAN_VIDEO} autoPlay loop muted playsInline />
      ...
      <div
        ...
        <div className="mt-1 text-3xl font-semibold tracking-tight text-white md:text-4xl" style={{ textShadow: '0 2px 20px rgba(0,0,0,.6)' }}>
          {building ? building.name : 'Meridian'}
        </div>
        <div className="mt-1 text-sm text-white/60">{building ? building.sub : 'Operations'}</div>
      </div>
    </div>
  );
}

window.MeridianScene = MeridianScene;
